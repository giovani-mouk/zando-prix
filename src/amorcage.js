...
//    si elle est vide : charge les données de démonstration (seed.sql),
//    sauf si DONNEES_DEMO=non ;
...
async function creerTablesSiBesoin() {
  ...
  // Données de démonstration : chargées tant que la table des produits est vide.
  // Vérifié à part de la création des tables : si un chargement précédent a
  // échoué (seed.sql est une transaction, donc annulé en entier), les tables
  // existent mais sont vides, et le démarrage suivant réessaie au lieu de
  // laisser le site tourner sans aucun produit.
  if (process.env.DONNEES_DEMO === 'non') return;
  const { rows: produits } = await pool.query('SELECT EXISTS (SELECT 1 FROM produits) AS rempli');
  if (produits[0].rempli) return;
